...
MicroView::MicroView(micro_oled_mode mode, uint8_t rst, uint8_t dc, uint8_t cs)
{
	if (mode == MODE_SPI)
...
void MicroView::setup(micro_oled_mode mode, uint8_t rst, uint8_t dc, uint8_t cs,
...
void MicroView::command(uint8_t c) {
	...
	if (interface == MODE_SPI)
	{
		...
		*ssport &= ~sspinmask;		// SS LOW
		spiTransfer(c); //MVSPI.transfer(c);
		*ssport |= sspinmask;	// SS HIGH
	}
	else if (interface == MODE_I2C)
	{
		*dcport &= ~dcpinmask;		// DC pin LOW		
		i2cWrite(I2C_ADDRESS_SA0_0, I2C_COMMAND, c);
	}
	else if (interface == MODE_PARALLEL)
	{
		parallelWrite(c, LOW);
...
void MicroView::data(uint8_t c) {
	...
	{
		*dcport &= ~dcpinmask;		// DC pin LOW
		i2cWrite(I2C_ADDRESS_SA0_0, I2C_DATA, c);
	}
	else if (interface == MODE_PARALLEL)
	{
